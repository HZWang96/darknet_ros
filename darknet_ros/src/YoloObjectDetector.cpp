...
detection* YoloObjectDetector::avgPredictions(network* net, int* nboxes) {
  ...
  detection* dets = get_network_boxes(net, buff_[0].w, buff_[0].h, demoThresh_, demoHier_, 0, 1, nboxes, 1);  // added input parameter '1'
  return dets;
}
...
void* YoloObjectDetector::detectInThread() {
  ...
  float* prediction = network_predict(*net_, X);  // changed from net_ to *net_
  ...
  draw_detections_v3(display, dets, nboxes, demoThresh_, demoNames_, demoAlphabet_, demoClasses_, 1);  // changed from draw_detections() to draw_detections_v3() and added input parameter '1'
...
void* YoloObjectDetector::displayInThread(void* ptr) {
  ...
  show_image_cv(buff_[(buffIndex_ + 1)%3], "YOLO");  // function changed because of change in image.h
  int c = cv::waitKey(waitKeyDelay_); // due to change in image.h
  if (c != -1) c = c % 256;
  if (c == 27) {
    demoDone_ = 1;
    return 0;
